...
m3_edit_module <- function(input, output, session, modal_title, car_to_edit, modal_trigger, tbl = "rct", data, sessionid) {
    ...
    showModal(
      modalDialog(
        ...
        conditionalPanel(
          ...
          # Bleeding
          conditionalPanel(
            'input.Event_Details_M3.includes("5")',
            ns = ns,
            tags$div(
              HTML(
                paste0(
          ...
          # Readmission
          conditionalPanel(
            'input.Event_Details_M3.includes("6")',
            ns = ns,
            tags$div(
              HTML(
                paste0(
            ...
            fluidRow(
              ...
              column(
                width = 3,
                conditionalPanel(
                  "input.Readmission_Type_M3 == 2", # if Other : Show text box.
                  ns = ns,
                  textAreaInput(
  ...
  observeEvent(input$CYfA, {
    updateRadioButtons(session, "Aspirin_M3", selected = 0)
    updateRadioButtons(session, "Trimetazidine_M3", selected = 0)
    updateRadioButtons(session, "Clopidogrel_M3", selected = 0)
    updateRadioButtons(session, "Nitrate_M3", selected = 0)
    updateRadioButtons(session, "Prasugrel_M3", selected = 0)
    updateRadioButtons(session, "Statin_M3", selected = 0)
    updateRadioButtons(session, "Ticagrelor_M3", selected = 0)
    updateRadioButtons(session, "BB_M3", selected = 0)
    updateRadioButtons(session, "ACE_M3", selected = 0)
    updateRadioButtons(session, "DPP4_M3", selected = 0)
    updateRadioButtons(session, "Wafarin_M3", selected = 0)
    updateRadioButtons(session, "NOAC_M3", selected = 0)
    updateRadioButtons(session, "Nicorandil_M3", selected = 0)
    updateRadioButtons(session, "Calcium_M3", selected = 0)
    updateRadioButtons(session, "Metformin_M3", selected = 0)
    updateRadioButtons(session, "Sulf_M3", selected = 0)
    updateRadioButtons(session, "Thia_M3", selected = 0)
    updateRadioButtons(session, "GLP_M3", selected = 0)
    updateRadioButtons(session, "Alpha_M3", selected = 0)
    updateRadioButtons(session, "Insulin_M3", selected = 0)
  })
