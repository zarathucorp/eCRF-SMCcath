...
m1_edit_module <- function(input, output, session, modal_title, car_to_edit, modal_trigger, tbl = "rct", data, sessionid) {
      ...
      modalDialog(
        ...
        conditionalPanel(
          ...
          radioButtons(
            ...
            selected = hold$Visit_M1, inline = T
          ),
        ...
        conditionalPanel(
          ...
          radioButtons(
            ...
            selected = hold$Reason_M1, inline = T
          ),
          conditionalPanel(
        ...
        textInput(
          ...
          "Systolic BP",
          value = ifelse(is.null(hold), NA, hold$SBP_M1)
        ),
        numericInput(
          ns("DBP_M1"), # Diastolic BP
          "Diastolic BP",
          value = ifelse(is.null(hold), NA, hold$DBP_M1),
          min = 0, max = 120,
          step = 1
        ),
        numericInput(
          ...
          "Heart Rate",
          value = ifelse(is.null(hold), NA, hold$HRT_M1),
          min = 0, max = 120,
          step = 1
        ),
        radioButtons(
          ...
          selected = hold$Cardiac_Status_M1, inline = T
        ),
        conditionalPanel(
        ...
        radioButtons(
          ...
          "Readmission",
          choices = c("Yes" = 0, "No" = 1),
          selected = hold$Readmission_M1, inline = T
        ),
        conditionalPanel(
        ...
        conditionalPanel(
          ...
          conditionalPanel(
            ...
            textInput(
              "Stent_Segment_M1",
              label = "Segment",
              value = ifelse(is.null(hold), NA, hold$Stent_Segment_M1)
            ),
            radioButtons(
              "Stent_Type_M1",
              label = "Type",
  ...
  observeEvent(input$ALLND_1, {
    updateTextInput(session, "SBP_M1", label = "Systolic BP", value = "ND")
  })
}
