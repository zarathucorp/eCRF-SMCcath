      ...
      showModal(
        modalDialog(
          ...
          fluidRow(
            column(
              width = 6,
              radioButtons(ns("DM_random"), "DM", c("No" = "0", "Yes" = "1"), inline = T),
            ),
            column(
              width = 6,
              radioButtons(ns("STEMI_random"), "AMI Type", c("NSTEMI", "STEMI"), inline = T),
...
(Target lesion not amenable for PCI by operators decision)", choices = c("Yes" , "No"), selected = NULL, inline = T),
          ...
          radioButtons(ns("ex_3"), "3.	다음 약제에 과민성이 있거나, 투약의 금기사항이 있는 경우
(aspirin, clopidogrel, ticagrelor, prasugrel, heparin, everolimus, zotarolimus, biolimus, sirolimus)", choices = c("Yes" , "No"), selected = NULL, inline = T),
          radioButtons(ns("ex_4"), "4. 조영제에 대한 아나필락시스의 기왕력이 있는 경우 (단순 알레르기 반응은 제외)", choices = c("Yes" , "No"), selected = NULL, inline = T),
          radioButtons(ns("ex_5"), "5. 임산부 및 수유부", choices = c("Yes" , "No"), selected = NULL, inline = T),
          radioButtons(ns("ex_6"), "6.	비 심장질환으로 기대 여명이 1년 미만이거나 치료에 순응도가 낮을 것으로 기대되는 자
(조사자가 의학적인 판단으로 정함)", choices = c("Yes" , "No"), selected = NULL, inline = T),
          radioButtons(ns("ex_7"), "7. 연구 참여를 거부한 환자", choices = c("Yes" , "No"), selected = NULL, inline = T),
